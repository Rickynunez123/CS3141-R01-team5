            ...
            <div class="container" style="max-width: 640px;">
                <div class="blob">
                    ...
                    <form name="createAccount" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
                        ...
                        <input type="radio" id="tenant" name="type" value="0">
                        <label for="type">Tenant</label>
                        <input type="radio" id="landlord" name="type" value="1">
                        <label for="type">Landlord</label>

                        <input type="submit" class="btn-login" value="Create Account" onclick="return CheckPasswords();">
                    </form><br>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
	        ...
	        if ($mysqli -> connect_errno) {
                ...
                if($result->fetch_row()[0] > 0) {
                    ...
                    //insertion string
                    $insertionString = "INSERT INTO Users (username, password, first_name, last_name, email, phoneNumber, type) VALUES ('$username', '$hPassword', '$firstName', '$lastName', '$email', '$phone', $type)";

                    //insert user into table and send verification Email
                    if ($mysqli->query($insertionString)) {

                        //Generate OTP for email verification
                        $otp = hash('sha256', $user . time() . $email);

                        $insertionString = "INSERT INTO OTP (otp, user, type) VALUES ('$otp', '$username', 0)";
            
                        //insert user into table and redirect if successful
                        if ($mysqli->query($insertionString)) {

                            $verificationLink = "http://smartments.rentals/verify.php?otp=" . $otp;
            
                            //send recovery email
                            $to = $email;
                            $subject = "Account Verification";
                            $message = '<p>Hello, please click here to verify your email: </p> <a href="' . $verificationLink . '">' . $verificationLink . '</a>';
